...
twoway_anova_test <- function(df, feature) {
    ...
    if (sig$p < 0.05) {
        ...
        print(sig)
        f <- as.formula(paste(feature, "~ expertiseLevel * domain + (1 | userID)"))
        print(f)
        m <- art(f, data = df)
        a <- anova(m)
        print(summary(a))
        ...
        o <- omega_squared(m)
        print(o)
        print(art.con(m, "expertiseLevel:domain", adjust = "holm") %>%
        summary() %>%
        ...
        model <- eval(parse(text = paste0("ezANOVA(data=df, dv=", feature,
            ", within = .(expertiseLevel, domain), wid = userID, detailed = TRUE)")))
        print(summary(model))
        anova_apa(model)
